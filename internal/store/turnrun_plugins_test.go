...
func pluginTurnRecord() store.TurnRunRecord {
	...
	rec.Plugins = []store.RunPluginPin{{Alias: "mail", Name: "smtp-send", Version: "1.0", Digest: "abc123"}}
...
func TestCreateTurnRunCarriesPluginLedgers(t *testing.T) {
	...
	if len(stmt.Args) != 19 {
		t.Fatalf("args = %d, want 19", len(stmt.Args))
	}
	...
	if got := stmt.Args[12]; !reflect.DeepEqual(got, []int64{1, 2}) {
		t.Errorf("call seq arg = %v", got)
	}
	if got := stmt.Args[16]; !reflect.DeepEqual(got, []string{"ok", "err"}) {
		t.Errorf("call outcome arg = %v", got)
	}
	if got := stmt.Args[18]; !reflect.DeepEqual(got, []string{"", "timeout"}) {
...
func TestDeadLetterTurnRunCarriesPluginLedgers(t *testing.T) {
	...
	if len(stmt.Args) != 21 {
		t.Fatalf("args = %d, want 21", len(stmt.Args))
	}
	if got := stmt.Args[10]; !reflect.DeepEqual(got, []string{"mail"}) {
		t.Errorf("pin alias arg = %v", got)
	}
	if got := stmt.Args[14]; !reflect.DeepEqual(got, []int64{1, 2}) {
		t.Errorf("call seq arg = %v", got)
	}
}

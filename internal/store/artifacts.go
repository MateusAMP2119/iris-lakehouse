...
// built binary's index row. The artifacts table holds content-addressed index
// entries -- hash PK, pipeline FK, size_bytes, recorded_at -- while the binary's
// bytes live only in the object store under the hash (objects.go), never as blobs
// in Postgres. Rows are immutable: the surface is insert-only, so a rebuild
// inserts a new row under its new hash and the pipeline's current artifact is
// simply its newest row. Rows leave only through artifact retirement (post-prune,
// a later E08 task) or teardown, never through an update.
...
// hash, owning pipeline, and byte size (building records the hash in artifacts,
// the bytes in the object store). It rides the single Writer like every meta
// write, and it only ever inserts -- artifact rows are immutable, so there is
// deliberately no update counterpart.

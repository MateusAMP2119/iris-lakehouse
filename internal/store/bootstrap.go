...
// lives in Postgres behind the single --pg-dsn: the embedded DDL is applied
// create-if-missing at install and re-checked at each leader election, with no
// self-migration ledger, no version gate, and no local state store (never SQLite,
// never .iris/state.db).
...
// full: the engine uninstall teardown. Dropping the database takes every control
// table -- and so every captured provenance row, endpoint, and access ledger
// entry -- with it. Like CREATE DATABASE it runs on the admin/maintenance
// connection, never on a connection to meta itself. IF EXISTS makes the teardown
// idempotent when meta was already dropped or never created.
...
// install path. cluster runs the CREATE DATABASE against the admin/maintenance
// database; meta runs the table DDL against the freshly-created meta database. In
// tests a single recorder can stand in for both to capture the whole bootstrap
// inventory in order.
...
// roster order. It is applied at bootstrap and re-checked at each leader
// election; the statements are idempotent, so a re-check emits the identical
// sequence and touches nothing that already exists. There is no ALTER, no DROP,
// no self-migration ledger, and no version gate: meta is engine memory, not
// migration-managed.

...
// This file is the dispatcher's snapshot-pin stamping (specification section 14). The
// pin is three values naming a run's inputs: at dispatch, the data database's LSN
// (snapshot_lsn) and journal high id (journal_floor); at the terminal transition, the
// journal high id again (journal_ceiling). Reading the LSN and the journal high id are
// data-database reads, not meta reads, so they enter through their own seams
// (LSNReader, JournalHighWatermark), fake-backed in tests; the stamped values then
// ride the single-writer run-record path.

// LSNReader reads the data database's current write LSN, the value pinned into a run's
// snapshot_lsn at dispatch (specification section 14). It is a data-database read seam,
// never a meta write; a real data-database-backed implementation and a fake both
// satisfy it.
...
// journal_floor at dispatch and journal_ceiling at the terminal transition
// (specification section 14). It is a data-database read seam, never a meta write.
...
// StampDispatch pins a run's input state at dispatch and mints the run record. It reads
// the data database's LSN into rec.SnapshotLSN and the journal high id into
// rec.JournalFloor -- the two dispatch-time reads of the snapshot pin (specification
// section 14) -- then hands the completed record to the run-record write path, so the
// created runs row carries the pin. Either data-database read failing aborts before any
// meta write, so a run is never minted with a half-formed pin.
...
// StampTerminal closes a run's journal window at its terminal transition. It reads the
// journal high id again -- the third and final read of the snapshot pin (specification
// section 14) -- and stamps it as the run's journal_ceiling, delimiting the window that
// opened at dispatch. The ceiling read happens here, at the terminal transition, not at

...
// This file holds the closed YAML-to-Postgres type mapping of specification
// section 5. The mapping is a total function on a closed set: every listed YAML
...
// (specification section 5). The parametrized forms varchar(n) and numeric(p,s)
...
// ResolveType maps one YAML type token to its Postgres type (specification
// section 5). A token outside the closed set -- including a bare varchar, an
...
func ResolveType(yamlType string) (string, error) {
	...
	return "", fmt.Errorf("unknown type %q (not in the closed type set of specification section 5)", yamlType)
}
...
// unknown type (specification section 5). Each reported error names the table,
